...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .reflection { position:absolute; left:50%; top:50.6%; height:30%; width:auto;
    transform:translateX(-50%) scaleY(-1); transform-origin:50% 82%;
    opacity:.14; filter:blur(4px); }

  #petit-eveille { position:absolute; left:50%; top:50.6%; height:30%; width:auto; transform:translateX(-50%); display:block; overflow:visible; }
  /* nom unique "actor-bob" : le labo (charCSS, inséré plus bas) définit aussi un
     @keyframes walk-bob SANS translateX(-50%) ; s'il gagnait, le perso perdait son
     centrage et se décalait à droite de son reflet pendant la marche. */
  @keyframes actor-bob { 0%,100%{transform:translateX(-50%) translateY(0)} 50%{transform:translateX(-50%) translateY(-1.8%)} }
  .actor.walking #petit-eveille { animation: actor-bob .5s ease-in-out infinite; }
  ...
  /* Étoiles d'ambiance (fond, discrètes) — taille/durée fixées en JS */
  .particles span { position:absolute; border-radius:50%;
    background:var(--tint); opacity:0; animation:float linear infinite; }
  @keyframes float { 0%{opacity:0; transform:translateY(8px)} 25%{opacity:.28} 75%{opacity:.2} 100%{opacity:0; transform:translateY(-14px)} }

  /* Particules d'émanation : naissent de la tête (neuro) / du torse (émotions) /
     des deux (éveil). Couleur, vitesse, direction et densité propres au mode (JS). */
  .emitp { position:absolute; border-radius:50%; pointer-events:none;
    transform:translate(-50%,-50%); animation:emit ease-out forwards; }
  @keyframes emit {
    0%   { opacity:0; transform:translate(-50%,-50%) scale(.3); }
    15%  { opacity:.95; }
    100% { opacity:0; transform:translate(-50%,-50%) translate(var(--dx,0), var(--dy,0)) scale(1); }
  }
  ...
  /* Soleil : noyau chaud + rayons tournants — overlay plein cadre (inset:0) pour
     ne JAMAIS influer sur la mise en page ; taille du disque relative au cadre. */
  .fx-sun { inset:0; }
  .fx-sun .disc { position:absolute; top:10%; right:7%; width:21%; aspect-ratio:1; }
  .fx-sun .core { position:absolute; inset:27%; border-radius:50%;
  ...
  /* Idée : ampoule au-dessus de la tête — overlay plein cadre, ampoule en % du cadre */
  .fx-idea { inset:0; }
  .fx-idea .bulb { position:absolute; left:50%; top:44%; width:11%; transform:translateX(-50%);
  ...
  /* Bulle de discussion — overlay plein cadre ; texte éditable depuis le panneau */
  .fx-bubble { inset:0; }
  .fx-bubble .box { position:absolute; left:60%; top:46%; max-width:34%; background:#fff; color:#1b1330; border-radius:14px;
    padding:9px 13px; font-size:clamp(11px,2.6vw,15px); font-weight:600; line-height:1.25; box-shadow:0 6px 22px rgba(0,0,0,.35); }
...
<div class="frame mode-eveil">
  ...
  <div class="fx fx-sun"><div class="disc"><div class="rays"></div><div class="core"></div></div></div>
  ...
  <div class="fx fx-idea"><div class="bulb"><svg viewBox="0 0 64 80" aria-hidden="true"><path d="M32 6 a22 22 0 0 1 13 39 c-3 3 -4 6 -4 10 H23 c0 -4 -1 -7 -4 -10 A22 22 0 0 1 32 6 Z" fill="#fff4c2" fill-opacity="0.20" stroke="#ffe08a" stroke-width="4" stroke-linejoin="round"/><line x1="24" y1="62" x2="40" y2="62" stroke="#ffe08a" stroke-width="4" stroke-linecap="round"/><line x1="26" y1="70" x2="38" y2="70" stroke="#ffe08a" stroke-width="4" stroke-linecap="round"/></svg></div></div>
  ...
  <div class="layer emit" id="emit"></div>
...
<script>
  (function () {
    ...
    // ----- Étoiles d'ambiance (fond, discrètes) -----
    var pc = document.getElementById('particles');
    for (var i = 0; i < 22; i++) {
      var s = document.createElement('span');
      var sz = (1.6 + Math.random()*2.2).toFixed(1);
    ...
    // ----- Particules d'émanation selon le mode -----
    //   neuro  -> tête : bleu, rapides, courtes, toutes directions (synapses)
    //   émotions -> torse : rose, lentes, qui montent doucement (ressenti)
    //   éveil  -> tête + torse : violet, nombreuses, qui rayonnent (conscience)
    // Origines en % du cadre, calées sur les spirales (tête ~56.5%, torse ~64%).
    var emitLayer = document.getElementById('emit');
    var EMIT = {
      'mode-neuro':    { color:'#8fb0ff', src:[[50,56.5]],         rate:130, size:[2,4], dur:[0.6,1.1], dist:[6,13],  angle:[0,360],   glow:7 },
      'mode-emotions': { color:'#ff93c4', src:[[50,64]],           rate:240, size:[3,6], dur:[2.2,3.4], dist:[10,20], angle:[248,292], glow:9 },
      'mode-eveil':    { color:'#b9a3ff', src:[[50,56.5],[50,64]], rate:110, size:[2,5], dur:[1.4,2.6], dist:[12,26], angle:[0,360],   glow:8 }
    };
    function rnd(a, b) { return a + Math.random() * (b - a); }
    function curMode() {
      if (frame.classList.contains('mode-neuro')) return 'mode-neuro';
      if (frame.classList.contains('mode-emotions')) return 'mode-emotions';
      return 'mode-eveil';
    }
    // décalage horizontal de l'acteur (marche) pour que les particules le suivent
    function actorShiftPct() {
      var t = getComputedStyle(actor).transform;
      var m = (t && t.indexOf('matrix') === 0) ? t.match(/matrix\(([^)]+)\)/) : null;
      if (!m) return 0;
      return (parseFloat(m[1].split(',')[4]) / (frame.clientWidth || 1)) * 100;
    }
    var lastEmit = 0;
    setInterval(function () {
      var conf = EMIT[curMode()]; if (!conf) return;
      var now = (window.performance ? performance.now() : Date.now());
      if (now - lastEmit < conf.rate) return;
      lastEmit = now;
      var H = frame.clientHeight; if (!H) return;
      var src = conf.src[(Math.random() * conf.src.length) | 0];
      var shift = actor.classList.contains('walking') ? actorShiftPct() : 0;
      var ang = rnd(conf.angle[0], conf.angle[1]) * Math.PI / 180;
      var dist = rnd(conf.dist[0], conf.dist[1]) / 100 * H;
      var sz = rnd(conf.size[0], conf.size[1]);
      var dur = rnd(conf.dur[0], conf.dur[1]);
      var p = document.createElement('span');
      p.className = 'emitp';
      p.style.left = (src[0] + shift + rnd(-2.2, 2.2)).toFixed(2) + '%';
      p.style.top  = (src[1] + rnd(-2, 2)).toFixed(2) + '%';
      p.style.width = sz.toFixed(1) + 'px';
      p.style.height = sz.toFixed(1) + 'px';
      p.style.background = conf.color;
      p.style.boxShadow = '0 0 ' + conf.glow + 'px ' + conf.color;
      p.style.setProperty('--dx', (Math.cos(ang) * dist).toFixed(1) + 'px');
      p.style.setProperty('--dy', (Math.sin(ang) * dist).toFixed(1) + 'px');
      p.style.animationDuration = dur.toFixed(2) + 's';
      emitLayer.appendChild(p);
      setTimeout(function () { p.remove(); }, dur * 1000 + 80);
    }, 45);
  })();
</script>
