...
// Overlay toujours monté, ouverture/fermeture en transitions CSS (remplace
// framer-motion AnimatePresence). Évite de charger le chunk framer sur la home.
export default function SearchOverlay({ isOpen, onClose }: SearchOverlayProps) {
  ...
  return (
    <div aria-hidden={!isOpen}>
      ...
      <div
        onClick={onClose}
        className={`fixed inset-0 z-[100] bg-black/75 backdrop-blur-sm transition-opacity duration-200 ${
      ...
      {/* Panel */}
      <div
        className={`fixed top-20 left-1/2 z-[101] w-full max-w-2xl px-4 transition-all duration-[250ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
        style={{
          transform: isOpen
            ? 'translateX(-50%) translateY(0) scale(1)'
            : 'translateX(-50%) translateY(-16px) scale(0.97)',
        }}
      >
